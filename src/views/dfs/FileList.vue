<template>
  <a-card :bordered="false">
    ...
    <s-table size="default" ref="table" rowKey="id" :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}" :columns="columns" :data="loadData"
      :rangPicker="range" defaultSort="createTime">
      <template v-for="col in ['fileName']" :slot="col" slot-scope="text, record">
        <div :key="col ">
          <a-input v-if="record.editable" style="margin: -5px 0" :value="text" @change="e => handleChange(e.target.value, record.id, col, record)" />
...
import { getFileList, delFile, saveFile } from '@/api/dfs'
export default {
  name: 'TableList',
  components: {
    STable,
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return getFileList(Object.assign(parameter, this.queryParam)).then(res => {
          return res
        })
      },
      selectedRowKeys: [],
      ...
      uploadUrl: process.env.VUE_APP_API_BASE_URL + '/dfs/file/upload',
      headers: {

<template>
  <a-form @submit="handleSubmit" :form="form" class="form">
    <a-row class="form-row" :gutter="24">
      <a-col :lg="{span: 6, offset: 6}" :md="12" :sm="24">
        <a-form-item>
          ...
          <a-select v-decorator="['tplCategory', {initialValue:'crud',rules: [{ required: true, message: '请选择生成模板' }]}]" @change="tplChange">
    ...
    <a-row class="form-row" :gutter="24">
      <a-col :lg="{span: 6, offset: 6}" :md="12" :sm="24">
        <a-form-item>
          ...
          <a-input placeholder="请输入生成模块名" v-decorator="[
              'moduleName',
    ...
    <a-row class="form-row" :gutter="24">
      ...
      <a-col :lg="{span: 6, offset: 1}" :md="12" :sm="24">
        <a-form-item>
          <span slot="label">主键类型
            <a-tooltip title="用作主键类型，例如 UUID,自增">
              <a-icon type="question-circle-o" />
            </a-tooltip>
          </span>
          <a-select v-decorator="['pkType', {initialValue:'UUID'}]">
            <a-select-option value="UUID">UUID</a-select-option>
            <a-select-option value="INCREMENT">自增</a-select-option>
          </a-select>
        </a-form-item>
      </a-col>
    </a-row>
...
export default {
  ...
  created () {
    ...
    this.$nextTick(() => {
      this.form.setFieldsValue(pick(this.mdl, 'packageName', 'tplCategory', 'moduleName', 'businessName', 'functionName', 'pkType', 'treeCode', 'treeParentCode', 'treeName'))
    })
  },
  methods: {
    tplChange (value) {

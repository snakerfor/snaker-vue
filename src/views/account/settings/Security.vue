  ...
  <a-list itemLayout="horizontal" :dataSource="data">
    <a-list-item slot="renderItem" slot-scope="item, index" :key="index">
      ...
      <template v-if="item.actions">
        <a slot="actions" @click="modifyPassword">{{ item.actions.title }}</a>
      </template>
    </a-list-item>
    <user-pwd-self-modal ref="modify"></user-pwd-self-modal>
  </a-list>
</template>

<script>
import UserPwdSelfModal from '@/views/system/modules/UserPwdSelfModal'
export default {
  components: {
    UserPwdSelfModal
  },
  data () {
    return {
      data: [
        { title: '账户密码', description: '当前密码强度', value: '强', actions: { title: '密码修改' } }
  ...
  methods: {
    modifyPassword () {
      this.$refs.modify.edit({ userId: this.$store.getters.userInfo.userId, loginName: this.$store.getters.userInfo.loginName })
    }
  }
}
</script>

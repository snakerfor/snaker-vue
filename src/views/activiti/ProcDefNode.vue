      ...
      <a-col :span="12">
        <div class="table-page-search-wrapper">
          <a-form @submit="handleSubmit" :form="form">
            ...
            <a-form-item label="选择角色" :labelCol="labelCol" :wrapperCol="wrapperCol" v-show="roleChecked">
              <!-- <a-select
              ...
              <a-select v-decorator="['roleIds', {rules: [{ required: roleChecked, message: '至少选择一个用户' }]}]" mode="multiple" allowClear placeholder="用户">
                <a-select-option v-for="(item, index) in roles" :key="index" :value="item.roleId">{{ item.roleName }}</a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item label="选择部门" :labelCol="labelCol" :wrapperCol="wrapperCol" v-show="deptChecked">
              <a-tree-select v-decorator="['deptIds', {rules: [{ required: deptChecked, message: '至少选择一个部门' }]}]" :dropdownStyle="{ maxHeight: '400px', overflow: 'auto' }"
                :treeData="depts" treeDefaultExpandAll @change="handleChangeDept" placeholder="部门" multiple allowClear>
              </a-tree-select>
            </a-form-item>
            <a-form-item label="选择用户" :labelCol="labelCol" :wrapperCol="wrapperCol" v-show="userChecked">
              <a-select v-decorator="['userIds', {rules: [{ required: userChecked, message: '至少选择一个用户' }]}]" mode="multiple" allowClear show-search placeholder="用户">

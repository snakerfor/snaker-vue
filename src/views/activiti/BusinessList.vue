<template>
  <a-card :bordered="false">
    ...
    <s-table size="default" ref="table" rowKey="id" :rowSelection="rowSelection" :columns="columns" :data="loadData" defaultSort="applyTime">
      ...
      <span slot="action" slot-scope="text, record">
        ...
        <!-- <a-divider type="vertical" /> -->
        <a v-if="false" :disabled="record.status===1" @click="delByIds([record.id])">删除</a>
      </span>
    </s-table>
    <business-modal ref="modal" @ok="handleOk" />
    <approve-modal ref="approvemodal" @ok="handleOk" :audit="false" />
...
import { getBusinessListMy, delProcessIns, delBusiness, resultMap, statusMap } from '@/api/activiti'
...
export default {
  ...
  methods: {
    ...
    handelAudit (record) {
      ...
      this.$refs.approvemodal.show({ businessKey: record.id, title: record.title })
    },
    handleOk () {
